...
def display_images(image, description):
    ...
    # (class_name, label, confidence), class_number = from_probability_get_label(Decode_model.densenet.NASNetMobile(include_top=True, weights='imagenet'), image)
...
class myThread (threading.Thread):
    ...
    def run(self):
        adv_x_iterative = create_adversarial_sample_iterative(self.model, self.input_image, self.input_label, self.epsilon, self.times)
...
class myThread_1 (threading.Thread):
    ...
    def run(self):
        adv_x_iterative = create_adversarial_sample_MI(self.model, self.input_image, self.input_label, self.epsilon, self.times)
        # q[self.name] = adv_x_iterative
        self.q[self.name] = adv_x_iterative
...
class myThread_2 (threading.Thread):
    ...
    def run(self):
        adv_x_iterative = create_adversarial_sample(self.model, self.input_image, self.input_label, self.epsilon, self.times)
        # q[self.name] = adv_x_iterative
        self.q[self.name] = adv_x_iterative
...
with open(file, 'w') as file_object:
    file_object.write(str(SAI_times_total))