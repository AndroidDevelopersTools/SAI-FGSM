    ...
    if eplison != 0:
        ...
        for i in range(times):
            ...
            if tf.norm(gradient, ord=2) != 0:
                input_image = alpha * (gradient / tf.norm(gradient, ord=2)) + input_image
            else:
                break
        return input_image
    else:
...
def create_adversarial_sample_MI(model, input_image, input_label, eplison, times):
    if eplison != 0:
        ...
        for i in range(times):
            ...
            input_image = input_image + alpha * (g / tf.norm(g, ord=2))
...
def create_adversarial_sample(model, input_image, input_label, epsilon, times):
    if epsilon != 0:
        ...
        while T >= Tmin:
            ...
            """
            if T >= Tmin:
                input_image = tf.clip_by_value(input_image, 0.5 * (tf.nn.tanh(below)+1), 0.5 * (tf.nn.tanh(above)+1))
            """
