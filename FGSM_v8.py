...
def display_images(image, description):
    ...
    (class_name, label, confidence), class_number = from_probability_get_label(
        tf.keras.applications.densenet.DenseNet121(include_top=True, weights='imagenet'), image)
    # plt.figure()
    # plt.imshow(image[0])
...
MI_times_total = []
SAI_times_total = []
for iterative_times in range(10):
    ...
    for j in range(1000):
        ...
        for i, eps in enumerate(epsilons):
            ...
            """
            adv_x_iterative = create_adversarial_sample_iterative(pretrained_model, current_img, current_label, eps,
                                                                  iterative_times + 1)
            ...
            adv_x_MI = create_adversarial_sample_MI(pretrained_model, current_img, current_label, eps,
                                                    iterative_times + 1)
